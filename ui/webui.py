...
def make_dash(
    ...
    app.layout = \
        html.Div(children=[html.H1(children='VK Group Analytics'
                 ), generate_table(groups, post_stats, member_stats,
                 info_stats), html.Div([html.Label('Plot type'),
                 dcc.Dropdown(id='drop', options=[
        ...
        ], value='likes'), html.Label('Groups'),
            dcc.Checklist(id='check', options=[{'label': name,
                          'value': i} for (i, name) in
                          enumerate(groups)], value=[0]),
            dcc.Graph(id='plot')])])

    stats = [parse_stats(get_group_stats(get_group_id(g),
             date_n_days_ago(100).timestamp())) for g in groups]
    dfs = [pd.DataFrame(stat) for stat in stats]
    for df in dfs:
        df.index = [date_n_days_ago(df.shape[0] - i).strftime('%d.%m.%Y'
                    ) for i in df.index]

    @app.callback(Output('plot', 'figure'), [Input('drop', 'value'),
                  Input('check', 'value')])
        ...
        if selected_type in ['age_visitors', 'age_reach']:
            ...
            if selected_grops:
                for group in selected_grops:
                    fig.add_bar(y=dfs[group].iloc[0, 12:19],
                                name=groups[group])
            ...
            if selected_grops:
                for group in selected_grops:
                    fig.add_scatter(y=dfs[group][selected_type],
                                    x=dfs[group].index,
                                    name=groups[group])
...
def generate_table(groups, *stats):
    ...
    for d in stats:
        ...
        for k in keys:
            table.append([k, *[str(st.get(k, None)) for st in d]])

    df = pd.DataFrame(table, columns=['feature'] + groups)
    return html.Table([html.Thead(html.Tr([html.Th(col) for col in
                      df.columns])),
                      html.Tbody([html.Tr([html.Td(df.iloc[i][col])
                      for col in df.columns]) for i in
                      range(len(df))])])
